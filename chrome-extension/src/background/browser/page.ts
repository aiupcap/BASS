...
export default class Page {
  ...
  async _updateState(useVision = true, focusElement = -1): Promise<PageState> {
    try {
      ...
      logger.warn('Current page is no longer accessible:', error);
      if (this._browser) {
  ...
  async refreshPage(): Promise<void> {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('timeout')) {
        logger.warn('Refresh timeout, but page might still be usable:', error);
      } else {
  ...
  async locateElement(element: DOMElementNode): Promise<ElementHandle | null> {
    if (!this._puppeteerPage) {
      // throw new Error('Puppeteer page is not connected');
      logger.warn('Puppeteer is not connected');
      return null;
    }
    ...
    for (const parent of iframes) {
      ...
      if (!frameElement) {
        ...
        logger.warn(`Could not find iframe with selector: ${cssSelector}`);
        return null;
      }
      ...
      if (!frame) {
        ...
        logger.warn(`Could not access frame content for selector: ${cssSelector}`);
        return null;
      }
      currentFrame = frame;
    }
